...
It also reports two things that are easy to miss by reading `add_noise`:
  ...
  * the gap between the nominal rate and the share of lines that actually change (a chosen
    confusion pair no-ops when `out.find(frm)` misses), and
  * per-pair applicability -- how often each hand-written pair could fire at all. One of the
    eleven cannot fire in practice.
"""
...
def main(argv=None):
    ...
    if all_pos:
        ...
        print("  (`add_noise` substitutes at the FIRST match, so this skew is an artifact of the\n"
              "   implementation rather than a property of any scanner)", file=w)
    return 0
