...
def load_surya(surya_dir):
    ...
    for sj in sorted(Path(surya_dir).glob("*_surya.json")):
        out.update(load_surya_file(sj))
    return out


def load_surya_file(sj):
    """One `*_surya.json` -> {image_filename: [line texts in reading order]}."""
    sj = Path(sj)
...
def index_surya(root):
    """Recursively index every `*_surya.json` under `root` by image stem -> path.

    Written 2026-09-20, replacing a `root/<gold slug>` lookup that matched NOTHING on the real
    tree. `directory-pipeline` names its output dirs after the scanned title, not after this
    repo's gold slug: franks1786's pages live in `ia_franks_1786_newyorkd/`, hearne1852's in
    `hearnes_brooklyn_city_directory_for_hearnesbrooklync1852/`. There is no rule mapping one to
    the other, and inventing an alias table would rot the first time a volume was re-run.

    `context.image` is the real join key and is already unique across the corpus -- it carries the
    IA identifier and the leaf number, or the CONTENTdm id for Tulsa. So index on it and let the
    gold rows say where they came from. Filenames only; nothing is parsed until a gold row asks
    for it, which matters at 11,387 files.
    """
    idx = {}
    for sj in Path(root).rglob("*_surya.json"):
        stem = sj.name[: -len("_surya.json")]
        for key in (stem, f"{stem}.jpg", f"{stem}.jpeg", f"{stem}.JPG", f"{stem}.png"):
            idx.setdefault(key, sj)
    return idx


def load_surya_indexed(index, wanted):
    """Parse only the indexed files the gold rows actually reference."""
    out, seen = {}, set()
    for key in wanted:
        sj = index.get(key)
        if sj is None or sj in seen:
            continue
        seen.add(sj)
        out.update(load_surya_file(sj))
    return out
...
def main(argv=None):
    ...
    index = None
    if a.surya_root:
        if not Path(a.surya_root).is_dir():
            ap.error(f"--surya-root {a.surya_root!r} is not a directory")
        index = index_surya(a.surya_root)
        print(f"indexed {len(index):,} surya keys under {a.surya_root}", file=sys.stderr)

    for gp in gold_paths:
        slug = _volume_slug(gp)
        rows = load_gold(gp)
        if index is not None:
            pages = load_surya_indexed(index, {r["image"] for r in rows if r.get("image")})
        else:
            if not Path(a.surya_dir).is_dir():
                print(f"  ! {slug}: no surya dir at {a.surya_dir}, skipped", file=sys.stderr)
                continue
            pages = load_surya(a.surya_dir)
        if not pages:
            print(f"  ! {slug}: no surya pages matched its {len(rows)} gold rows, skipped",
                  file=sys.stderr)
            continue
        st = measure(rows, pages, a.min_sim)
