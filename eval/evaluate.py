...
def metrics(res: dict) -> dict:
    ...
    for f in FIELDS:
        ...
        f1s.append(f1)
        per[f] = {"em": round(100 * d["em"] / n, 1), "p": round(prec, 3),
                  "r": round(rec, 3), "f1": round(f1, 3), "gold_ne": d["gold_ne"]}
    return {"n": res["n"], "row_exact_pct": round(100 * res["row_exact"] / n, 1),
            "macro_f1": round(sum(f1s) / len(f1s), 3) if f1s else 0.0, "per_field": per}


def report(res: dict) -> dict:
    m = metrics(res)
    print(f"\n{'field':<18} {'EM%':>6} {'P':>6} {'R':>6} {'F1':>6}  (gold_ne)")
    print("-" * 56)
    for f in FIELDS:
        pf = m["per_field"][f]
        print(f"{f:<18} {pf['em']:>6.1f} {pf['p']:>6.2f} {pf['r']:>6.2f} {pf['f1']:>6.2f}  ({pf['gold_ne']})")
    print("-" * 56)
    print(f"rows={m['n']}  whole-row exact match={m['row_exact_pct']:.1f}%  macro-F1={m['macro_f1']:.3f}")
    return m
