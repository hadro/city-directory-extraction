...
> the pipeline; measured 2026-09-20, and it reversed its own top two recommendations), and
...
data_prep/
  ...
  ocr_delta.py                  # gold raw_line vs the Surya text it was corrected FROM:
                                #   recovers the post-OCR parallel corpus and measures it.
                                #   4,325 pairs; the confusion table is NOT usable as-is --
                                #   see docs/POST_OCR_CORRECTION.md §5
  ...
  measure_synth_noise.py        # --noise knob -> input CER, so train-time and production OCR
                                #   quality sit on one axis. Run before AND after any change
                                #   to add_noise. See docs/POST_OCR_CORRECTION.md §2
